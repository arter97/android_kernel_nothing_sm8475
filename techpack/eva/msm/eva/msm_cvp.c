	...
	unsigned int in_buf_num)
{
	...
	if (map_type == MAP_PERSIST) {
		...
		rc = eva_msm_cvp_map_user_persist(inst, in_pkt, offset, buf_num, fd_arr);
	} else if (map_type == UNMAP_PERSIST) {
		rc = eva_msm_cvp_unmap_user_persist(inst, in_pkt, offset, buf_num);
	} else {
	...
	if (rc) {
		...
		if (map_type == MAP_PERSIST) {
			...
			rc = eva_msm_cvp_unmap_user_persist(inst,
